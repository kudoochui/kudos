...
func (c *OneClient) SetSelector(nodeName string, s Selector) {
	c.mu.Lock()
	c.selectors[nodeName] = s
	if xclient, ok := c.xclients[nodeName]; ok {
...
func (c *OneClient) Go(ctx context.Context, nodeName string, servicePath string, serviceMethod string, session protocol.ISession, args interface{}, reply interface{}, done chan *Call) (*Call, error) {
	...
	if xclient == nil {
		...
		xclient = c.xclients[nodeName]
		if xclient == nil {
			xclient, err = c.newXClient(nodeName)
			c.xclients[nodeName] = xclient
		}
		c.mu.Unlock()
		if err != nil {
			return nil, err
...
func (c *OneClient) newXClient(nodeName string) (xclient XClient, err error) {
	...
	d, err := c.discovery.Clone(nodeName)
	if err != nil {
		return nil, err
	}

	if c.serverMessageChan == nil {
		xclient = NewXClient(nodeName, c.failMode, c.selectMode, d, c.option)
	} else {
		xclient = NewBidirectionalXClient(nodeName, c.failMode, c.selectMode, d, c.option, c.serverMessageChan)
	}
	...
	if s, ok := c.selectors[nodeName]; ok {
		xclient.SetSelector(s)
	}
...
func (c *OneClient) Broadcast(ctx context.Context, servicePath string, serviceMethod string, args interface{}, reply interface{}) error {
	...
	return xclient.Broadcast(ctx, servicePath, serviceMethod, args, reply)
}
...
func (c *OneClient) Fork(ctx context.Context, servicePath string, serviceMethod string, args interface{}, reply interface{}) error {
	...
	return xclient.Fork(ctx, servicePath, serviceMethod, args, reply)
}
