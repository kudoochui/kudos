...
type ServiceDiscovery interface {
	...
	Clone(nodeName string) (ServiceDiscovery, error)
...
func NewXClient(nodeName string, failMode FailMode, selectMode SelectMode, discovery ServiceDiscovery, option Option) XClient {
	client := &xClient{
		failMode:     failMode,
		selectMode:   selectMode,
		discovery:    discovery,
...
func NewBidirectionalXClient(nodeName string, failMode FailMode, selectMode SelectMode, discovery ServiceDiscovery, option Option, serverMessageChan chan<- *protocol.Message) XClient {
	client := &xClient{
		failMode:          failMode,
		selectMode:        selectMode,
		discovery:         discovery,
...
func (c *xClient) Call(ctx context.Context, servicePath,serviceMethod string, session protocol.ISession, args interface{}, reply interface{}) error {
	...
	if share.Trace {
		log.Debugf("select a client for %s.%s, failMode: %v, args: %+v in case of xclient Call", servicePath, serviceMethod, c.failMode, args)
	}
	...
	if share.Trace {
		log.Debugf("selected a client %s for %s.%s, failMode: %v, args: %+v in case of xclient Call", client.RemoteAddr(), servicePath, serviceMethod, c.failMode, args)
	}
...
func (c *xClient) wrapCall(ctx context.Context, client RPCClient, servicePath,serviceMethod string, session protocol.ISession, args interface{}, reply interface{}) error {
	...
	if share.Trace {
		log.Debugf("call a client for %s.%s, args: %+v in case of xclient wrapCall", servicePath, serviceMethod, args)
	}
	...
	if share.Trace {
		log.Debugf("called a client for %s.%s, args: %+v, err: %v in case of xclient wrapCall", servicePath, serviceMethod, args, err)
	}
...
// FailMode and SelectMode are meanless for this method.
func (c *xClient) Fork(ctx context.Context, servicePath, serviceMethod string, args interface{}, reply interface{}) error {
	if c.isShutdown {
		return ErrXClientShutdown
	}
